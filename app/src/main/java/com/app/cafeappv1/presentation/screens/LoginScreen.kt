...
fun LoginScreen(
    ...
    Box(
        ...
        Column(
            ...
            // ── CAMPO CELULAR (funciona como contraseña) ──────────
            Column(modifier = Modifier.fillMaxWidth()) {
                Text(
                ...
                OutlinedTextField(
                    ...
                    onValueChange = { vm.onCelularChange(it) },
                    placeholder   = { Text("Ingrese su celular", color = CoffeeMuted, fontSize = 14.sp) },
                    isError       = uiState.celularError,
                    supportingText = {
                        if (uiState.celularError)
                            Text("Ingrese su celular", color = CoffeeGold, fontSize = 12.sp)
                        else
                            Text("Es el mismo con el que te registraste", color = CoffeeMuted, fontSize = 11.sp)
                    },
                    visualTransformation = if (uiState.mostrarCelular)
                        VisualTransformation.None else PasswordVisualTransformation(),
                    keyboardOptions = KeyboardOptions(keyboardType = KeyboardType.Phone),
                    trailingIcon = {
                        IconButton(onClick = { vm.toggleMostrarCelular() }) {
                            Icon(
                                imageVector = if (uiState.mostrarCelular)
                                    Icons.Filled.Visibility else Icons.Filled.VisibilityOff,
                                contentDescription = "Mostrar celular",
                                tint = CoffeeMuted
                            )
                        }
                    },
